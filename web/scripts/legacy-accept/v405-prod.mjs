...
(async () => {
  ...
  await p.waitForTimeout(2600); const btn0 = await p.$('#submitBtn'); if (btn0) { await p.waitForFunction(() => { const x = document.querySelector('#submitBtn'); return x && !/(d)s/.test(x.textContent || ''); }, { timeout: 10000 }).catch(() => {}); await btn0.click().catch(() => {}); await p.waitForTimeout(1000); } await p.fill('#t', 'mneme'); await p.click('#f button');
  ...
  const probe = await p.evaluate(() => {
    ...
    const rects = [...document.querySelectorAll('.sig')].map(el => ({ ...el.getBoundingClientRect(), src: el.getAttribute('src') }));
    ...
    for (let i = 0; i < rects.length; i++) for (let j = i + 1; j < rects.length; j++) {
      if (hit(rects[i], rects[j]) > 1) { overlapBad++; const a = rects[i], c = rects[j]; console.log('擦边:', Math.round(hit(a, c)) + 'px²', a.src?.slice(-8) || '', c.src?.slice(-8) || ''); }
    }
